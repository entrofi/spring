...
public class UserAspectJoinPoint {
    ...
    public void logginAdvice(JoinPoint joinPoint) {
        System.out
                .println("Before advice with joinPoint passed as an argument says, "
                        + "'We are calling logginAdvice on the method:"
                        + joinPoint.toString() + "'");
        System.out
     ...
     * @param string
     */
    @Before("net.entrofi.sportslogtracker.springaop.user.aspect.UserAspectPointcut"
            + ".allMethodsPointcut()&& args(firstname,..) ")
