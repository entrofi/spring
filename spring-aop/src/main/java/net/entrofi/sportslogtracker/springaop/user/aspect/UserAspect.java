 ...
 *		<li>
 *			<em>getNameAdvice()</em> advice will execute for any Spring Bean method with signature <code>public String getFirstname()</code>. This is a very important point to remember, if we will create Employee bean using new operator the advices will not be applied. Only when we will use ApplicationContext to get the bean, advices will be applied.</li>
 *		<li>We can use asterisk (*) as wild card in Pointcut expressions, <em>getAllAdvice()</em> will be applied for all the classes in <code>net.entrofi.sportslogtracker.springaop.user.service</code> package whose name starts with <code>get</code> and doesn’t take any arguments.</li>
 *	</ul>
 *
