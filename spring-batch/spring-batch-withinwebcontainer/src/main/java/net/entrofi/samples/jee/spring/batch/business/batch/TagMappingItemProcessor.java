...
public class TagMappingItemProcessor implements ItemProcessor<TagMappingItem, ProductTag> {
    ...
    public ProductTag process(TagMappingItem item) throws Exception {
        ...
        ProductTag.ProductTagId tagId = new ProductTag.ProductTagId(item.getTag(), merchant);
        ProductTag tag = Optional.ofNullable(this.entityManager.find(ProductTag.class, tagId))
                .orElseGet(() -> new ProductTag(tagId));
